...
Copy this template into `.atk/runner/filter_abnormal.py` when using the `atk-filter-rules` Skill. Replace every `TODO_AGENT_TUNING` block with the confirmed rule logic. The user runs this script manually.
...
Reads current .atk/results/vN/results.csv and overwrites current
