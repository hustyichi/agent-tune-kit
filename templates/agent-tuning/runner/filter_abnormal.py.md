...
Copy this template into `agent-tuning/runner/filter_abnormal.py` when using the `atk-filter-rules` Skill. Replace every `TODO_AGENT_TUNING` block with the confirmed rule logic. The user runs this script manually.

```python
#!/usr/bin/env python3
"""Rule-based abnormal-case filter for Agent tune kit.

Generated by the atk-filter-rules Skill.
