...
Copy this template into `agent-tuning/runner/test_runner.py` and replace every `TODO_AGENT_TUNING` block after the `atk-setup` Skill has inspected the target Agent and dataset. The generated script should stay low-dependency and project-local.

```python
#!/usr/bin/env python3
"""Project-local Agent tuning test runner.

Generated by the atk-setup Skill.
- Preserves all original dataset columns and order.
- Appends required actual-output column: agent_output.
- Writes results incrementally to agent-tuning/results/vN/results.csv.
...
def execute_rows(
    ...
    *,
    ...
    yielded in completion order so results.csv remains useful during long runs.
    """
    if concurrency == 1:
