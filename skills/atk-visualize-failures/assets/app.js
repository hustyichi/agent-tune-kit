(function () {
  ...
  function renderTable() {
    ...
    setText("page-label", "第 " + (state.page + 1) + " / " + pages + " 页");
  ...
  function renderStats() {
    ...
    detail.appendChild(el("span", { class: "badge" + (role ? " gt" : ""), text: role ? ROLE_LABELS[role] : meta.type }));
    ...
    meta.sampleValues.forEach(function (value) { samples.appendChild(el("span", { class: "badge", text: truncate(value, 80) })); });
  ...
  function buildLengthBuckets(field) {
    ...
    var buckets = [];
    for (var i = 0; i < 8; i++) buckets.push({ min: i * step, max: (i + 1) * step - 1, count: 0 });
    lengths.forEach(function (len) { buckets[Math.min(7, Math.floor(len / step))].count += 1; });
