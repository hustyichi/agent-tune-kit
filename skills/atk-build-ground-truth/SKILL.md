...
- Optional local private tuning context: `.atk/context.md`, especially `Ground Truth Standard`.
...
- Optional `.atk/context.md` update limited to user-confirmed `Ground Truth Standard` changes.
...
4. Read `.atk/context.md` if it exists. Apply only user-confirmed `Ground Truth Standard` guidance; ignore dataset
   metadata, run summaries, or generated statistics if present.
5. Inspect headers and representative rows to identify input fields, expected-like source fields, existing `ground_truth` values, and any business context columns.
6. Ask the user to select one global ground-truth style before generating values when the local context and user request
   do not already define it:
   - exact answer; or
   - natural-language acceptance criteria.
7. Determine whether safe generation is possible:
...
8. Generate row-level `ground_truth` values following the selected global or dataset-wide style.
9. Before writing, present a candidate modification summary and ask for explicit confirmation if writing would overwrite existing `ground_truth`, normalize existing `ground_truth`, semantically replace expected-like fields, or conflict with `.atk/context.md` standards. For large datasets, include affected row counts and representative examples instead of every changed row.
10. Write `.atk/datasets/dataset.csv` only after required confirmations are satisfied.
11. If the user confirmed a new or refined `ground_truth` standard, create or update only the `Ground Truth Standard`
    section in `.atk/context.md`. Do not record dataset headers, field types, row counts, result paths, or routine
    generation details there.
...
- `.atk/context.md` contains a `Ground Truth Standard` that conflicts with the requested enrichment;
