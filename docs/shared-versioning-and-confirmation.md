...
- abnormal criteria, expected-result columns, or pass/fail semantics are ambiguous;
- current/previous-version sample matching is unreliable;
- an existing `filter_abnormal.py` should be reused or updated;
- overwriting `abnormal_cases.csv`, `report.md`, or `tuning_plan.md` would discard user edits not generated by the flow.
...
- `agent-tuning-report`: require current `results.csv` and `abnormal_cases.csv`; `app.log` is optional. If previous version lacks `tuning_plan.md` or sample matching is unreliable, degrade to single-version or lower-confidence report with explicit explanation, not silent failure.
- `agent-tuning-apply-tuning`: require current `report.md`; if missing, stop and tell the user to run report generation first. After changes, write `tuning_plan.md` with the exact headings `## 目标异常清单`, `## 调优手段`, and `## 关联改动`. Suggest user git commits/checkpoints; do not perform automatic rollback/baseline restore.
