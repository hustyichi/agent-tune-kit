...
- `atk-find-failures`: require current `vN/results.csv`; if expected-result columns or failure criteria are ambiguous, ask for judgment. It writes `abnormal_cases.csv` in the current version and states that the file is overwritten.
