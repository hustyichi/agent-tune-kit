...
- Optional row logs: `.atk/results/vN/logs/row_{source_index:06d}.log`, referenced from `agent_output_log_path`, for configured same-process Python `logging` records emitted under an active ATK row context
...
- `atk-find-failures`: require current `vN/eval_results.csv`; if expected-result columns or failure criteria are ambiguous, ask for judgment. It writes `failure_cases.csv` in the current version and states that the file is overwritten. After writing `failure_cases.csv`, tell the user to run `atk-report` next.
...
- `atk-visualize-dataset`: no version directory is required; it operates on the non-versioned `.atk/datasets/` directory. Require `.atk/datasets/dataset.csv`; if it is missing or its header cannot be parsed, stop with guidance to run `atk-build-dataset` first. Generate `.atk/datasets/dataset.html` only through the fixed plugin-owned stdlib script `scripts/generate_dataset_browser.py`, with escaped dependency-free static HTML, input-vs-ground_truth comparison for confirming ground_truth correctness, dataset quality lint, search/filter/pagination, schema-adaptive role switching, and client-side review export. Confirm before overwriting an existing `dataset.html` that may contain user edits. This Skill does not read or write any `.atk/results/vN` version directory.
