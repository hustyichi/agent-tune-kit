...
- an existing `find_failures_by_rule.py` should be reused or updated;
...
- `atk-find-failures-by-rule`: require current `vN/eval_results.csv`; if no current version or missing `eval_results.csv`, stop with repair/rerun guidance. If existing `.atk/runner/find_failures_by_rule.py` exists, ask whether to reuse or update rule logic. This Skill generates or updates the script and instructs the user to run it manually; it does not run `find_failures_by_rule.py` itself in the normal PRD flow.
