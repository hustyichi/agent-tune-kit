...
- `skills/atk-visualize-dataset/SKILL.md` — run the fixed stdlib generator script to write `.atk/datasets/dataset.html` as an optional, dependency-free browser for `.atk/datasets/dataset.csv`, focused on confirming whether each ground_truth matches expectations.
...
3. Trigger `atk-status` to inspect state and route to the right stage.Trigger `atk-status` to inspect state and route to the right stage.
   - Optionally trigger `atk-visualize-dataset` after the dataset exists to render `.atk/datasets/dataset.csv` into a single-file offline `.atk/datasets/dataset.html`. The Skill calls `skills/atk-visualize-dataset/scripts/generate_dataset_browser.py` to produce deterministic HTML with input-vs-ground_truth comparison, dataset quality lint, search/filter/pagination, schema-adaptive role switching, and client-side review export, with no LLM summaries, project-local template, dependencies, or sidecar metadata. It operates on the non-versioned `.atk/datasets/` directory and confirms before overwriting an existing `dataset.html`.
