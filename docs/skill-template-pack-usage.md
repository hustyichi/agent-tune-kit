...
- `skills/atk-build-ground-truth/SKILL.md` — enrich an existing `.atk/datasets/dataset.csv` with a canonical `ground_truth` column before result-version work.
- `skills/atk-tune-ground-truth/SKILL.md` — tune existing `.atk/datasets/dataset.csv` `ground_truth` values from `dataset_review.csv` exported by `atk-visualize-dataset`.
- `skills/atk-visualize-dataset/SKILL.md` — run the fixed stdlib generator script to write `.atk/datasets/dataset.html` as an optional, dependency-free Dataset Visualizer-style static browser for `.atk/datasets/dataset.csv`, focused on reviewing rows and expected-result fields.
...
1. Prepare the local Agent service and evaluation dataset. If you only have a business description, examples, or acceptance rules, trigger `atk-build-dataset` first. It asks 1-3 targeted questions when input fields or expected behavior are unclear, then writes `.atk/datasets/dataset.csv` directly with `atk_id`. It must not infer a canonical `ground_truth` column unless the user explicitly supplies `ground_truth`, correct answers, or a clear correct-result judgment policy. Existing `dataset.csv` requires overwrite confirmation; the first version does not merge, append, or create a candidate dataset workflow. After dataset creation, it routes the next step by Agent availability: use `atk-init` when an Agent exists, use `atk-new-agent` when no Agent exists, and mention both paths when that cannot be confirmed.
2. If `.atk/datasets/dataset.csv` already exists but expected-result semantics are weak, trigger `atk-build-ground-truth` before result-version work. It requires valid `atk_id`, asks for one dataset-wide style (`exact answer` or `natural-language acceptance criteria`), preserves existing columns, and writes or updates canonical `ground_truth` only after any overwrite/semantic-replacement risk is confirmed with a candidate modification summary containing affected row counts and representative examples. It does not create `.atk/results/vN`, run the Agent/eval runner, or write `failure_cases.csv`; rerun `atk-run` if existing `eval_results.csv` predates enrichment.
   - If a reviewer exported `dataset_review.csv` from `atk-visualize-dataset`, trigger `atk-tune-ground-truth` to use `review_feedback` as user judgment for correcting existing `ground_truth`. It prefers `.atk/datasets/dataset_review.csv`, falls back to the newest structurally valid matching `dataset_review*.csv` in Downloads, uses no fingerprint validation, and writes corrected `ground_truth` values back to `.atk/datasets/dataset.csv` while preserving columns.
3. Optionally trigger `atk-visualize-dataset` after the dataset exists to render `.atk/datasets/dataset.csv` into a single-file offline `.atk/datasets/dataset.html`. The Skill calls `skills/atk-visualize-dataset/scripts/generate_dataset_browser.py` to produce deterministic Dataset Visualizer-style HTML with Data List / Field Feature Analysis tabs, column visibility controls, sorting/pagination, row detail drawer, input-vs-ground_truth comparison, dataset quality lint, schema-adaptive role switching, and client-side review export, with no LLM summaries, project-local template, user-installed frontend dependencies, or sidecar metadata. It operates on the non-versioned `.atk/datasets/` directory and confirms before overwriting an existing `dataset.html`.
4. Trigger `atk-init` in Codex. The Skill reads the Agent source and dataset, asks only about unsafe ambiguity, validates/normalizes `.atk/datasets/dataset.csv` as the ATK canonical runnable dataset with a required `atk_id` column, then writes `.atk/runner/eval_runner.py`.
5. Trigger `atk-run`. It executes `python3 .atk/runner/eval_runner.py`; the runner creates or reuses a version directory and writes `.atk/results/vN/eval_results.csv` plus optional `app.log`. After a failure set exists, `atk-run --only-failures` reruns only rows whose `atk_id` appears in the latest prior `failure_cases.csv`, selecting those rows from `.atk/datasets/dataset.csv` rather than executing the result CSV as input. When same-process Python `logging` capture is configured, global logging records are written to `.atk/results/vN/app.log`; row-attributed records also create `.atk/results/vN/logs/row_{source_index:06d}.log` files and record relative POSIX paths in `log_path` for serial runs and for `--concurrency > 1` while the generated concurrent row-log flag is enabled. The row router only writes records emitted under an active ATK row context; stdout/stderr, subprocess, multiprocess, and post-row background logs remain out of row-log scope. If concurrent row logging is disabled, `--concurrency > 1` visibly downgrades and no row logs are created while global Python logging still writes `app.log`.
6. Choose one failure-finding entry:
...
7. Trigger `atk-report` to create `.atk/results/vN/report.md`. From `v2` onward, it compares the current version with the previous existing version and reads the previous `tuning_plan.md` when available. Reports prefer row-specific files referenced by `log_path` and fall back to `app.log` when row logs are unavailable.
8. Optionally trigger `atk-visualize-failures` to create `.atk/results/vN/failure_cases.html`. It can run any time current `failure_cases.csv` exists; same-version `report.md` is optional best-effort context and never blocks CSV visualization. The Skill calls `skills/atk-visualize-failures/scripts/generate_failure_browser.py` to produce deterministic HTML with expected-vs-actual review, search/filter/pagination, schema-adaptive role switching, and no LLM summaries, project-local template, dependencies, or sidecar metadata.
9. Trigger `atk-tune` to change the Agent and write `.atk/results/vN/tuning_plan.md`.
10. Optionally create a user git commit/checkpoint. Agent tuning rollback remains user-git-only guidance; this plugin does not automate Agent code restore.
11. Run the same loop again. The next test run creates `v{N+1}` when the current max version already has `eval_results.csv`.
